...
- 差し戻しで設計を改訂する場合は、`review.md` の Finding と `implementation.md` の Plan Deviations を読んでから改訂する。Round 1 は消さず、`## Round {n}` に Revision Notes と変更後の節を書く
...
初回は `# Architecture: {task_id}` から全体を `## Round 1` に書き、差し戻しで改訂する場合は末尾に `## Round {n}` を追記する。
...
## Round 2
（差し戻し時に追記。Round 1 は直接直さず、変更点を書く）
...
- 書く: `architecture.md` のみ。改訂時は Round 1 を消さず、`## Round {n}` に Revision Notes と変更後の節を再掲する（Implementer の Plan Deviations が参照する箇所を消さない）
